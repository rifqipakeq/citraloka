            ...
            <Container>
                <div className='mb-4 flex items-center justify-between gap-4'>
                    ...
                    <div className='w-full md:w-4.6'>
                        <Search url={route('tickets.index')} placeholder={'Search tickets data by name...'} filter={filters}/>
                    </div>
                </div>
                    ...
                    <Table>
                        ...
                        <Table.Tbody>
                            {tickets.data.map((ticket, i)=>(
                                <tr key={{ i }}>
                                    <Table.Td>{++i + (tickets.current_page-1) * tickets.per_page}</Table.Td>
                                    <Table.Td>{ticket.ticket_code}</Table.Td>
                                    <Table.Td>{ticket.name}</Table.Td>
                                    <Table.Td>Rp{ticket.price_per_pack.toLocaleString()}</Table.Td>
                                    <Table.Td>{ticket.qty}</Table.Td>
                                    <Table.Td>
                                        <div className='flex item-center gap-2'>
                                            {hasAnyPermission(['tickets edit']) &&
                                                <Button type={'edit'} url={route('tickets.edit', ticket.id)}/>
                                            }
                                            {hasAnyPermission(['tickets delete']) &&
                                                <Button type={'delete'} url={route('tickets.destroy', ticket.id)}/>
                                            }
                                        </div>
                                    </Table.Td>
                                </tr>
                ...
                <div className='fle item-center justity-center'>
                    {tickets.last_page !== 1 && (<Pagination links={tickets.links}/>)}
                </div>
            </Container>
        </AuthenticatedLayout>
    )
